...
const (
	...
	backlinkSectionHeaderImplements = "## Implemented By"
...
func HasBacklinkSection(content string) bool {
	return strings.Contains(content, backlinkSectionHeader) || strings.Contains(content, backlinkSectionHeaderImplements)
}
...
func UpdateBacklinks(filePath string, repoRelativePath string, repoRoot string, backlinks map[parser.DocumentType][]parser.BacklinkInfo) error {
	...
	// Determine which header to use based on file type
	isControl := strings.Contains(repoRelativePath, "/controls/")
	var sectionHeader string
	if isControl {
		sectionHeader = backlinkSectionHeaderImplements
	} else {
		sectionHeader = backlinkSectionHeader
	}

	// Check if file has the backlink section
	...
	// Split content at the backlink section
	parts := strings.Split(fileContent, sectionHeader)
	if len(parts) != 2 {
		return fmt.Errorf("expected exactly one '%s' section in file", sectionHeader)
	}
	...
	newContent := beforeSection + sectionHeader + "\n\n" + backlinkContent
...
func generateBacklinkContent(targetFilePath string, repoRoot string, backlinks map[parser.DocumentType][]parser.BacklinkInfo) string {
	...
	// Check if target is a control file - if so, exclude framework backlinks
	// (framework mappings are in the Framework Mapping section, not Referenced By)
	isControl := strings.Contains(targetFilePath, "/controls/")
	...
	// For control files, exclude framework backlinks
	if isControl {
		types = []parser.DocumentType{
			parser.TypeStandards,
			parser.TypeProcesses,
...
func EnsureBacklinkSection(filePath string) error {
	...
	// Determine which header to use based on file path
	isControl := strings.Contains(filePath, "/controls/")
	var sectionHeader string
	if isControl {
		sectionHeader = backlinkSectionHeaderImplements
	} else {
	...
	if strings.Contains(content, backlinkSectionHeader) || strings.Contains(content, backlinkSectionHeaderImplements) {
